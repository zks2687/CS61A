...
def about(topic):
    ...
    def contain(paragraph):
        para_words = split(lower(remove_punctuation(paragraph)))
        for word in topic:
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for word in valid_words:
            diffs.append(diff_function(user_word, word, limit))
        if min(diffs) > limit:
            return user_word
        else:
...
def fastest_words(game):
    ...
    for i in words:
        this_word_times = [time(game, j, i) for j in players]
        index = min(players, key = lambda x: this_word_times[x])
        # min = time(game, 0, i)
        # index = 0
        # for j in players:
        #     if time(game, j, i) < min:
        #         min = time(game, j, i)
        #         index = j
        fastest_words_list[index].append(word_at(game, i))
...
def key_distance_diff(start, goal, limit):
    ...
    if limit < 0:
        ...
        add = 1 + key_distance_diff(start, goal[1:], limit - 1)
        remove = 1 + key_distance_diff(start[1:], goal, limit - 1)
        substitute = key_distance[start[0], goal[0]] + key_distance_diff(start[1:], goal[1:], limit - 1)
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    if key in had_autocorrect_words:
        return had_autocorrect_words[key]
    else:
        ...
        if min(diffs) > limit:
            ...
            result = valid_words[diffs.index(min(diffs))]
        had_autocorrect_words[key] = result
        return result
    # END PROBLEM EC2
