...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        ...
        # print("DEBUG: %d", score0)
        # print("DEBUG: %d", score1)
        who = other(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        highest, last = running_high, last_score
        if who == 0:
            if score0 - last_score > running_high:
                print("%d point(s)! That's the biggest gain yet for Player 0" %(score0 - last_score))
                highest = score0 - last_score
            last = score0
        else:
            if score1 - last_score > running_high:
                print("%d point(s)! That's the biggest gain yet for Player 1" %(score1 - last_score))     
                highest = score1 - last_score
            last = score1
        # print("DEBUG: ", last_score, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    number, res, max = 1, 1, 0
    while number < 11:
        cur_score = averaged(number, dice)
        if cur_score > max:
            max = cur_score
            res = number
        number += 1
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    q = opponent_score % 10
    r = (opponent_score // 10) % 10
    bacon_score = score + 10 - q + r
    if abs(bacon_score % 10 - q) == r:
        if bacon_score < opponent_score:
            return 0
        else:
            return num_rolls
    elif bacon_score - score >= cutoff:
        return 0 
    else:
        return num_rolls
    # END PROBLEM 11
