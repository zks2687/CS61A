...
def examples():
    t = mobile(arm(1, planet(2)),
               arm(2, planet(1))) # weight = 3
    u = mobile(arm(5, planet(1)),
               arm(1, mobile(arm(2, planet(3)),
                              arm(3, planet(2))))) # weight = 6
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'can not divide zero'
...
def quadratic(x, a, b, c):
    ...
    if x1 <= extreme_point <= x2 and a != 0:
        if a > 0:
            return interval(f(extreme_point), max(f(x1), f(x2)))
        else:
            return interval(min(f(x1), f(x2)), f(extreme_point))
    else:
