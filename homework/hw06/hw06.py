...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.funds < self.price:
            ...
            exchange = self.funds - self.price
            self.funds = 0
            self.num -= 1
...
class Coin:
    ...
    def worth(self):
        sub = Mint.current_year - self.year
        return self.cents + (sub - 50 if sub > 50 else sub)
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            ...
            greater_son = t.branches[1] if len(t.branches) == 2 else t.branches[0]
            return max(bst_max(greater_son), t.label)
    
    def bst_min(t):
        if t.is_leaf():
            return t.label
        else:
            smaller_son = t.branches[0]
            return min(bst_min(smaller_son), t.label)
    ...
    if t.is_leaf():
        ...
        print('DEBUG: ', bst_min(right), bst_max(left))
...
def store_digits(n):
    ...
    if cnt(n) == 1:
        ...
        q = n // (10 ** (cnt(n) - 1))
        r = n % (10 ** (cnt(n) - 1))
        # print('DEBUG: ', cnt(n), q, r)
        return Link((q), store_digits(r))
