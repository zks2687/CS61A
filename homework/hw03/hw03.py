...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        "*** YOUR CODE HERE ***"
        def h(x):
            return func(g(x)) # apply the most recent function first
        return composer(h)
...
def g_iter(n):
    ...
    i = 1
    while i < n:
        last, cur, next = cur, next, next + 2 * cur + 3 * last
        i += 1
    return last 
...
def count_change(total):
    ...
    def count_change_helper(total, smallest_coin):
        if total == 0:
            ...
            with_small = count_change_helper(total - smallest_coin, smallest_coin)
            no_small = count_change_helper(total, 2 * smallest_coin)
            return with_small + no_small
