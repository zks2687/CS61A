...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return subseq_helper(s[1:], prev) # exclude s[0]
        else:
...
def make_glookup(class_assignments):
    ...
    stu_grades = 0
    all_grades = 0
    def cal(assignment, grade):
        nonlocal stu_grades
        nonlocal all_grades
        stu_grades += grade
        all_grades += class_assignments[assignment]
        return stu_grades / all_grades
    return cal
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        return 1
    return 2 * (2 * n - 3) * num_trees(n - 1) // n
